...
cls & echo [3/5] 正在将 Wwise 音频转换为 OGG（文件较多时需要一些时间）...
set "WMODE=%~1"
...
ping -n 1 -w 800 192.0.2.1 >nul
cls & echo [4/5] 正在修复 OGG 索引并移动文件...
FOR %%d IN (Tools\Decoding\*.OGG) DO (%SHOW%    修复 %%~nxd & "Tools\revorb.exe" "%%d" %OUT% & MOVE "%%d" "MP3" %OUT%)
cls & echo [5/5] 正在通过 FFmpeg 转码为 MP3...
FOR %%e IN (MP3\*.OGG) DO (%SHOW%    转码 %%~nxe & "Tools\ffmpeg.exe" -i "%%e" -acodec libmp3lame -q:a 0 -y "MP3\%%~ne.mp3" %OUT% & DEL "%%e")
ping -n 1 -w 800 192.0.2.1 >nul & cls
echo -------------------------------------------------------------

echo 解包完成！文件已保存到「MP3」文件夹
